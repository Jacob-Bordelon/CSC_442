...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog='timelock.py',conflict_handler="resolve")
    parser.add_argument("-e",'--epoch', help="Store the epoch time")
    parser.add_argument('-c',"--current",help="Store the current time")
    parser.add_argument('--full',help="Show the full hash",action="store_true")
    args = parser.parse_args()
    # Get the epoch time from stdin
    epochInput = args.epoch if args.epoch != None else sys.stdin.read()
   
    # get the current system time
    # NOTE Change c to change the current time
    #/////////////////////////////////////////////////////////
    currentTime = args.current if args.current != None else datetime.now().strftime("%Y %m %d %H %M %S") #datetime.now().strftime("%Y %m %d %H %M %S") # use for normal timelock.py
    ...
    #sys.stdout.write("\n"+hashed)
    #sys.stdout.write(hashed[len(hashed)/2])
